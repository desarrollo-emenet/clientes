...
export class DocPdfService {
  ...
  constructor(private calculo: CalculoService){}
  ...
  async descargarEstadoCuentaPDF(pservicios: any, pinfoCliente: any): Promise<void> {
    const estadoCuenta = pservicios.estadoCuenta;
    const periodoActual = this.calculo.getMesPagoReciente(pservicios) || this.obtenerPeriodoActual();
    const itemData = {
    ...
    const cliente = pinfoCliente;
    const servicios = pservicios;
    const numeroCliente = pinfoCliente.cliente;
    const pageW = doc.internal.pageSize.getWidth();
    const margen = 12;
    const anchoUtil = pageW - margen * 2;
    const totalServicios = this.calcularTotalServicios(servicios.estadoCuenta);
    ...
    await this.dibujarEncabezadoPDF(doc, pageW, margen, itemData, pservicios);
    ...
    y = this.dibujarTablaServiciosPDF(
      doc, servicios, cliente, estadoCuenta ?? [], y, margen, anchoUtil, pinfoCliente
    );
    y = this.dibujarResumenTotalPDF(
      doc, cliente, itemData, totalServicios, y, margen, anchoUtil, pservicios
    );
  ...
  private dibujarInfoEncabezado(
    doc: jsPDF, pageW: number, margen: number, item: any, pservicios: any
  ): void {
    const hoy = new Date().toLocaleDateString('es-MX', {
      year: 'numeric', month: 'long', day: 'numeric'
    });
    const x = pageW - margen - 56;
    const periodo = this.calculo.getMesPagoReciente(pservicios) || this.obtenerPeriodoActual();
    doc.setFontSize(7.5);
    doc.setFont('helvetica', 'bold');
  ...
  private async dibujarEncabezadoPDF(
    ...
    this.dibujarInfoEncabezado(doc, pageW, margen, item, pservicios);
    await this.dibujarCodigoBarrasEncabezado(doc, pageW, margen);
  }

  private calcularTotalServicios(estadoCuenta: any): number {
    return estadoCuenta.reduce((acc: number, mes: any) => {
      return Number(mes.importe) + acc;
    }, 0);
  }
  ...
  private dibujarTablaServiciosPDF(
    ...
    const importeMensual = this.calcularTotalServicios(servicios.estadoCuenta);

    const deudaCliente = pinfoCliente.deuda ?? 0;
  ...
  private dibujarVentasTrimestralesPDF(
    ...
    ultimos3.forEach((item, idx) => {
      ...
      doc.setTextColor(20, 20, 20);
      doc.text(
        `$ ${this.formatearPesos(item.importe)}`,
    ...
    // --- Total trimestral ---
    const totalTrimestral = importeMensual;
  ...
  private dibujarResumenTotalPDF(
    ...
    const mes = item?.mensualidad
      || this.calculo.getMesPagoReciente(pservicios)
